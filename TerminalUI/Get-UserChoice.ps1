...
function Get-UserChoice {
    param(
        # The list of strings or UserChoiceItems to choose between
        [object[]] $items,
    ...
    $items = $items | ForEach-Object {
        if ($_ -is [UserChoiceItem]) {
            ...
            return [UserChoiceItem]::new([string] $_)
        }
    }
    ...
    function Write-Menu {
        ...
        for ($i = 0; $i -lt $items.length; ++$i) {
            ...
            $text = $items[$i].ToMenuString($selection[$i], $Multi.IsPresent)
            if ($i -eq $active_line) {
...
class UserChoiceItem {
    ...
    UserChoiceItem([string] $name) {
        $this.Name = $name
        $this.Value = $name
        $this.Description = ''
    }
    UserChoiceItem([string] $name, [object] $value) {
        $this.Name = $name
        $this.Value = $value
        $this.Description = ''
    }
    UserChoiceItem([string] $name, [object] $value, [string] $description) {
        $this.Name = $name
        $this.Value = $value
    ...
    [string] ToMenuString([switch] $Selected, [switch] $Multi) {
        ...
        if ($this.Description) {
            $outp = $outp + "  $($global:PSStyle.Foreground.BrightBlack)($($this.Description))$($global:PSStyle.Reset)"
        }
        return $outp
    }
}
